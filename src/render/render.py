...
class SimpleNet(torch.nn.Module):
    def __init__(self):
        ...
        self.fc1 = torch.nn.Linear(2, 64)  # 输入维度为2 (x, y)，输出维度可以根据需要调整
        self.fc2 = torch.nn.Linear(64, 1)  # 输出维度为1 (灰度值)
...
# 渲染函数
def render_image(net,size=128):
    # 生成坐标
    coords = generate_coords(size)
    # 使用网络渲染图像
    with torch.no_grad():
        outputs = net(coords)

    # 调整输出形状为图像大小
    rendered_image = outputs.view(size, size)
...
if __name__ == '__main__':
    image_size = 128  # 图像大小
    net = SimpleNet()  # 实例化网络模型
    rendered_image = render_image(image_size, net)  # 渲染图像

    # 可以根据需要，将渲染得到的图像输出或进一步处理
